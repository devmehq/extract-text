const fs = require('fs'),
  ...
  test = (_testFunction, withMime) => {
    ...
    const _test = function (ext, name, _text) {
      it(`will ${ext} files`, (done) => {
        const docPath = path.join(__dirname, 'files', name),
          textBuff = fs.readFileSync(docPath);

        testFunction(withMime ? mime.getType(docPath) : docPath, textBuff, (error, text) => {
