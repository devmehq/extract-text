"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.runExecIntoFile = exports.getTextFromZipFile = exports.unzipCheck = exports.createExecOptions = exports.yauzlError = exports.replaceBadCharacters = void 0;
var child_process_1 = require("child_process");
var path_1 = __importDefault(require("path"));
var fs_1 = __importDefault(require("fs"));
var os_1 = __importDefault(require("os"));
var outDir = path_1.default.join(os_1.default.tmpdir(), 'textract');
var replacements = [
    [/[\u201C|\u201D|]|â€œ|â€/g, '"'],
    [/[\u2018|\u2019]|â€™|â€˜]/g, "'"],
    [/â€¦/g, '…'],
    [/â€“|â€”/g, '–'], // long hyphen
];
var rLen = replacements.length;
// Up front creation of tmp dir
if (!fs_1.default.existsSync(outDir)) {
    fs_1.default.mkdirSync(outDir);
}
// replace nasty quotes with simple ones
function replaceBadCharacters(text) {
    var i, repl;
    for (i = 0; i < rLen; i++) {
        repl = replacements[i];
        text = text.replace(repl[0], repl[1]);
    }
    return text;
}
exports.replaceBadCharacters = replaceBadCharacters;
function yauzlError(err, cb) {
    var msg = err.message;
    if (msg === 'end of central directory record signature not found') {
        msg = "File not correctly recognized as zip file, ".concat(msg);
    }
    cb(new Error(msg), null);
}
exports.yauzlError = yauzlError;
function createExecOptions(type, options) {
    var execOptions = {};
    if (options[type] && options[type].exec) {
        execOptions = options[type].exec;
    }
    else if (options.exec) {
        execOptions = options.exec;
    }
    return execOptions;
}
exports.createExecOptions = createExecOptions;
function unzipCheck(type, cb) {
    (0, child_process_1.exec)('unzip', function (error /* , stdout, stderr */) {
        if (error) {
            // eslint-disable-next-line no-console
            console.error("textract: 'unzip' does not appear to be installed, so textract will be unable to extract ".concat(type, "."));
        }
        cb(error === null);
    });
}
exports.unzipCheck = unzipCheck;
function getTextFromZipFile(zipfile, entry, cb) {
    zipfile.openReadStream(entry, function (err, readStream) {
        var text = '';
        var error = '';
        if (err) {
            cb(err, null);
            return;
        }
        readStream.on('data', function (chunk) {
            text += chunk;
        });
        readStream.on('end', function () {
            if (error.length > 0) {
                cb(error, null);
            }
            else {
                cb(null, text);
            }
        });
        readStream.on('error', function (_err) {
            error += _err;
        });
    });
}
exports.getTextFromZipFile = getTextFromZipFile;
/**
 * 1) builds an exec command using provided `genCommand` callback
 * 2) runs that command against an input file path
 *   resulting in an output file
 * 3) reads that output file in
 * 4) cleans the output file up
 * 5) executes a callback with the contents of the file
 *
 * @param {string} label Name for the extractor, e.g. `Tesseract`
 * @param {string} filePath path to file to be extractor
 * @param {object} options extractor options as provided
 *   via user configuration
 * @param {object} execOptions execution options passed to
 *   `exec` commmand as provided via user configuration
 * @param {function} genCommand function used to generate
 *   the command to be executed
 * @param {string} cb callback that is passed error/text
 *
 */
function runExecIntoFile(label, filePath, options, execOptions, genCommand, cb) {
    // escape the file paths
    var fileTempOutPath = path_1.default.join(outDir, path_1.default.basename(filePath, path_1.default.extname(filePath)));
    var escapedFilePath = filePath.replace(/\s/g, '\\ ');
    var escapedFileTempOutPath = fileTempOutPath.replace(/\s/g, '\\ ');
    var cmd = genCommand(options, escapedFilePath, escapedFileTempOutPath);
    (0, child_process_1.exec)(cmd, execOptions, function (error /* , stdout, stderr */) {
        if (error !== null) {
            error = new Error("Error extracting [[ ".concat(path_1.default.basename(filePath), " ]], exec error: ").concat(error.message));
            cb(error, null);
            return;
        }
        fs_1.default.exists("".concat(fileTempOutPath, ".txt"), function (exists) {
            if (exists) {
                fs_1.default.readFile("".concat(fileTempOutPath, ".txt"), 'utf8', function (error2, text) {
                    if (error2) {
                        error2 = new Error("Error reading".concat(label, " output at [[ ").concat(fileTempOutPath, " ]], error: ").concat(error2.message));
                        cb(error2, null);
                    }
                    else {
                        fs_1.default.unlink("".concat(fileTempOutPath, ".txt"), function (error3) {
                            if (error3) {
                                error3 = new Error("Error, ".concat(label, " , cleaning up temp file [[ ").concat(fileTempOutPath, " ]], error: ").concat(error3.message));
                                cb(error3, null);
                            }
                            else {
                                cb(null, text.toString());
                            }
                        });
                    }
                });
            }
            else {
                error = new Error("Error reading ".concat(label, " output at [[ ").concat(fileTempOutPath, " ]], file does not exist"));
                cb(error, null);
            }
        });
    });
}
exports.runExecIntoFile = runExecIntoFile;
